<script lang="ts">
  import { getUserNamespaces, createProject, createCommitFromArc, type Namespace, ApiError, getExistingProjects, sanitizeProjectName } from "@/lib/gitlab";
  ...
  async function tryExport() {
    try {
      ...
      if (existingProject) {
        logger.push({ message: `Project name ${projectName} already exists`, urlToRepo: existingProject.http_url_to_repo, success: "FAIL" });
