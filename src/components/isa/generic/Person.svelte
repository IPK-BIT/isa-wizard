<script lang="ts">
    ...
    onMount(()=>{
        if (!person.comments.find((c: {name: string, value: string}) => {return c.name === 'Person ID'})) {
            person.comments = [...person.comments, {name: 'Person ID', value: ''}];
        }
    });

    let orcid: string = $derived(
        person.comments.find((c: {name: string, value: string}) => {return c.name === 'Person ID'})?.value || '',
    );

    $effect(()=>{
        if (person && orcid !== person.comments.find((c: {name: string, value: string}) => {return c.name === 'Person ID'})?.value) {
            let idx = person.comments.findIndex((c: {name: string, value: string}) => {return c.name === 'Person ID'});
            if (idx !== -1) {
...
<section>
    <div class="container">
        ...
        <div class="person-viewer">
            ...
            <div class="controls">
                <button class="btn btn-secondary" onclick={()=>{mode='edit'}}>Edit</button>
                {#if countPeople > 1}
        ...
        <div class="person-editor">
            <div>
                ...
                <div style="gap: .5rem; display: flex; flex-direction: column;">
                    ...
                    <div class="orcid-wrapper">
                        <span>{orcid}</span>
                        <button class="btn btn-warning" onclick={() => { orcid = ''; }}>Remove</button>
                    </div>
                    {:else}
                    <Svelecte
                        placeholder="ORCID"
                        ...
                        fetchProps={{ headers: { 'Content-Type': 'application/vnd.orcid+json' } }}
            ...
            <div class="controls">
                <button class="btn btn-secondary" onclick={()=>{mode='view'}}>Stop Editing</button>
                {#if countPeople > 1}
