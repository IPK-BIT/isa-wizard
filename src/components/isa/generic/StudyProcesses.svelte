<script lang="ts">
  ...
  function loadTable(data: { column_mapping: any; dataframe: DataFrame; selectedProtocols: any[] }) {
    console.log("Table loaded", data);
    let column_mapping = data.column_mapping;
    let dataframe = data.dataframe;
    let protocols = data.selectedProtocols;

    console.log($state.snapshot(protocols));

    let rowProcesses: any[] = [];

    protocols.forEach((protocol, i) => {
      let isFirst = i === 0,
        isLast = i === protocols.length - 1;

      // Create Process Names based on protocol name
      ...
      if (!isLast) {
        // Name of the next process is not available yet, but can be derived from the next protocol name
        process.nextProcess = { "@id": "#Process_" + protocols[i + 1].name.replace(/\s+/g, "_") };
      }
...
{#if !(value.processSequence.length > 0)}
  <TableUpload approve={(data: any) => loadTable(data)} bind:value />
{:else}
