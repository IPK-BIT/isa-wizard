...
<div class="server-container">
  <div>
    ...
    <button
      class="btn large"
      onclick={async () =>
        (allStudies = (await fetchBrapiData("studies")) ?? [])}>Load</button
    >
  </div>
</div>

{#if allStudies.length > 0}
  <p>{allStudies.length} Studies found</p>
  ...
  <label>
    ...
    >
      {#each allStudies as study}
        <option value={study.studyDbId} class="option"
          >{study.studyDbId} - {study.studyName}</option
        >
      {/each}
    </select>
  </label>
