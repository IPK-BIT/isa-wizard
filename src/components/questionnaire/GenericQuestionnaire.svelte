...
<section>
    ...
    <div role="button" tabindex="0" aria-pressed="false">
      {#key currentStep}
        ...
        {#if steps[currentStep].component}
          <ComponentWrapper component={componentTypes[steps[currentStep].component]} jsonPath={steps[currentStep].jsonPath} componentConfig={steps[currentStep].componentConfig} />
        {/if}
      {/key}
    </div>
  </div>
  ...
  <div class="controls">
    ...
    {:else}
      <button class="btn large float-right" onclick={closeWizard}>Finish</button>
    {/if}
  </div>
</section>
