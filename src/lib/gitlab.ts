...
export async function getExistingProjects(config: Config, access_token: string) {
    const response = await fetch(`${config.base_url}/api/v4/projects?owned=true`, {
        method: "GET",
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
    ...
    const data = await response.json() as GitProjectResponse[];
